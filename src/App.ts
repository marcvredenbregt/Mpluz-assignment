...
export const App = async (delayBetweenActions: number) => {
  ...
  const encoders = Array.from({ length: 7 }, () =>
    generateRandomDongle(ipPrefix, 'encoder'),
  ) as Encoder[];
  const decoders = Array.from({ length: 6 }, () =>
    generateRandomDongle(ipPrefix, 'decoder'),
  ) as Decoder[];
  ...
  console.log(
    `OR-1 - Initial state - connecting encoders and decoders to the router`,
  );
  console.log(
    `==============================================================================================`,
  );
  ...
  console.log(
    ` - Connect source to input 1 of encoder 1 model ${encoders[0].model}`,
  );
  encoders[0].connectInput(1);
  console.log(
    ` - Connect source to input 1 of encoder 3 model ${encoders[2].model}`,
  );
  encoders[2].connectInput(1);
  console.log(
    ` - Connect source to input 1 of encoder 5 model ${encoders[4].model}`,
  );
  encoders[4].connectInput(1);
  console.log(
    ` - Connect sources to inputs of encoder 8 model ${encoders[7].model}`,
  );
  encoders[7].connectInput(1);
  encoders[7].connectInput(2);

  console.log(
    ` - Connect monitor to output 1 of decoder 1 model ${decoders[0].model}`,
  );
  decoders[0].connectOutput(1);
  console.log(
    ` - Connect monitor to output 1 of decoder 2 model ${decoders[1].model}`,
  );
